...
public class LightFlickerPulse : MonoBehaviour {  // This name 'LightFlickerPulse' is a very convenient name, trust me!
	...
	private Color[] matMainColor = new Color[49];
			...
			if(outsideLight.Length > 0 && outsideLight.Length < 50 && useOutsideLight){
				for(int i = 0; i < outsideLight.Length; i++){
					if(outsideLight[i] == null && displayScriptErrors && !error2){
						...
						if (soundClip != null)
						{
							outsideLight [i].gameObject.AddComponent (typeof(AudioSource));
							AudioSource a = outsideLight [i].GetComponent<AudioSource> ();
							a.clip = soundClip;
							a.loop = true;
							a.spatialBlend = 1f;
							a.minDistance = 1f;
	...
	{
		if (matRenderer.Length > 0)
		{
			for (int i = 0; i < matRenderer.Length; i++)
			{		
				matMainColor [i] = matRenderer [i].material.color; 
				matRenderer [i].material.EnableKeyword ("_EMISSION");
				if (useType == UseType.Color)
					matRenderer [i].material.SetColor ("_EmissionColor", matMainColor [i] * z);

				if (useType != UseType.Color)
					matRenderer [i].material.color = matMainColor [i];
			}
		}
	}
